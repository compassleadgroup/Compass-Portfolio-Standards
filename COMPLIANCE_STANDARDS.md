...
**The consent block names only the channels the form actually collects.** The standard lead form is six fields and collects an email address (BUILD_PLAYBOOK.md SECTION 2A, operator instruction 2026-09-14), so the six-field template below is the default. Narrowing consent to the channels collected is always safe; widening it is never safe. Never promise contact by a channel the form does not capture, and never collect a channel the consent does not name. **Those two failures are the same defect seen from opposite sides, and both are live defects: a form that collects an email under a phone-only consent is contacting people on a channel they did not agree to.** Change the field and the consent in one commit, never in two.

**TCPA consent copy template, six-field form (the default; collects phone and email):**
> "By submitting this form, I agree that [SITE_BRAND] and a [SERVICE] contractor may contact me at the phone number and email address I provided, including by automated phone calls and prerecorded messages, regarding [SERVICE] services. Consent is not a condition of any purchase. Message and data rates may apply. I may opt out at any time by asking to be removed during any call or by replying to any email. See our Privacy Policy."

**TCPA consent copy template, fallback, only for a form that genuinely collects no email address:**
...
**No consent template names text messaging, and no site may add it back (operator instruction 2026-08-11, unchanged by the 2026-09-14 field change).** The portfolio's published numbers do not send or receive texts, so consenting to a channel nobody uses is a claim the business cannot support, and the "reply STOP to any text" opt-out pointed at a mechanism that does not exist. Both came out of every template and every site on 2026-08-11. The opt-out is asking to be removed during any call, plus replying to any email now that every form collects one. **Adding email back does not reopen texting:** email is a channel the form captures and the business actually uses, texting is neither.

**What this costs, recorded rather than hidden.** Consent covers calls and email, so neither the site operator nor the contractor receiving a lead may text the homeowner. A contractor who texts a lead is outside the consent this site obtained. If that becomes a problem in practice, the fix is an operator decision to restore the channel in these templates and re-sweep the portfolio, not a per-site edit.
...
**Closed 2026-09-14.** That mismatch is gone in the other direction: every form now collects an email, so the email template is the correct one everywhere and the fallback exists only for a form that genuinely has no email field. The lesson the 2026-08-03 note recorded still holds and is the reason the rule above is now stated in both directions: whichever way the field and the consent drift apart, they are fixed in one commit, centrally.
...
- **A caption, or any visible text, that describes a photograph.** Operator instruction, 2026-09-22: "Delete all text that describes images. Never do that again on any site." No caption under a photo, no line over it, no sentence in body copy narrating what the picture shows, and no "illustrative photograph" or "not a job we arranged" disclaimer. A reader can see the picture. Text that repeats it is filler that makes the page longer and the site look amateur. The description belongs in the `alt` attribute, which is not visible, is what a screen reader and a crawler read, and stays required. Two things are not captions and are not covered: the key, legend, units or source attribution on a chart, map or diagram, which is the graphic's data and stays; and a heading or link that happens to sit inside a `figcaption` element, which is page content in the wrong element and gets moved, not deleted.
