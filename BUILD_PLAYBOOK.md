...
1. **Exactly six visible fields: name, phone, email, city, what the job is, and when they want it built** (operator instruction 2026-09-14, which replaced the four-field ask). Phone and email are both required, so a lead that does not answer the phone is still reachable, and the endpoint's spam scoring stops charging 50 points for a submission with neither. Nothing else goes on: these six and no seventh.

1b. **Every visible field is required** (operator instruction 2026-09-18, swept across all 61 sites the same day). Not just phone and email: name, the location field, the job or situation dropdown, the timeline dropdown, any free-text notes box, and the TCPA consent box. The only fields that stay unrequired are hidden tracking inputs and the spam honeypot, and a required honeypot would block every real submission. **Expect submissions per visitor to fall again, on top of the six-field drop.** Same trade as 1a: this portfolio is paid on contracts that close, and a partly filled lead a contractor cannot act on is not worth what the raw count implies. Reversing is a portfolio re-sweep on measured volume, the operator's call.

1a. **What the four-field rule bought, and what the operator traded it for.** The retired rule was evidence-backed: a phone field alone costs 30 to 48 percent of completions, and every added required field costs more. The trade is deliberate and belongs to the money model rather than to conversion rate. This portfolio earns a commission on contracts that close, not on leads delivered, and a lead with two contact channels and a stated timeline is worth more to a contractor than a lead with one channel and no timeline. **Expect submissions per visitor to fall. That is the known price, not a defect to fix by quietly dropping a field.** If it falls far enough to hurt, the decision to reverse is the operator's, made on measured form volume, and it is a portfolio re-sweep rather than a per-site edit.
2. **Every dropdown leads with an empty placeholder option, and keeps a neutral answer inside the list.** The placeholder is `<option value="" disabled selected>Choose one</option>`; the neutral answer, for example "Not sure yet", stays as a real selectable option below it. Both parts matter. A dropdown with no neutral default silently submits its first real option for every visitor who never touches it, which the contractor reads as a stated fact, and a dropdown whose default already holds a real value makes `required` do nothing at all. That second half is the 2026-09-18 correction: before the sweep, every timeline dropdown in the portfolio defaulted to "Not sure yet", so marking it required would have enforced nothing.

2a. **The timeline control is a dropdown, never a free-text box or a date picker.** A free-text box returns unsortable prose and a date picker asks for a precision nobody has before a contractor has seen the lot. Five options plus the neutral default, worded so the contractor can sort by them: as soon as possible, within three months, three to six months, more than six months out, and pricing it for a budget. That last one is the valuable one: it tells a contractor which leads to call last without the homeowner having to say so.
3. **Check required flags and select defaults, not just the field count.** One live site had phone optional while email was required. Under item 1b every visible field is required, so the defect to look for now is any visible control without `required`, and any dropdown marked required whose default option carries a real value, which passes validation without the visitor ever choosing. The 2026-09-18 sweep made 228 such fixes across 61 sites, and 10 of those sites had a TCPA consent box that was not required at all. **Sweep the portfolio by branch name, not by default branch:** two site repos default to a `claude/scaffold-*` docs branch while the live site is on `main`, so a sweep that clones default branches skips them and reports a clean run.

3a. **Every form carries a spam honeypot, and it has to be genuinely invisible.** The lead endpoint scores a filled honeypot at 100, its single strongest signal, and a form without one gives it nothing to score. Take the field out of the layout and collapse it to an empty box on the input itself, not only on its wrapper: an ancestor with `overflow: hidden` clips what a person sees but does not change the child's layout box, so a honeypot collapsed only at the wrapper still measures as a real field and will trip a tap-target or field-count check. Keep it out of the tab order and off autocomplete, and prefer that to `display: none`, which some bots read as a field to skip. **And never mark it required.** Item 1b makes every visible field required, and the 2026-09-18 sweep that applied it also put `required` on the invisible honeypot on three sites (alamometalbuildings.com, thegutterreport.com, woodfloorreport.com). An empty required input the visitor cannot see or focus fails browser validation, so every real submission on those three sites was blocked for two days until it was found and fixed on 2026-09-20. A render check cannot see this by counting fields or by visibility; it has to fill every visible field and ask the browser whether the form validates (`form.checkValidity()`), and it has to assert the honeypot carries no `required` attribute. The Central Texas render-check.mjs does both; carry that check into every future build.
...
13. **The consent block names exactly the channels the form collects, and never names text messaging** (operator instruction 2026-08-11, unchanged). The form now collects an email, so the consent names the phone number and the email address, and the opt-out is asking to be removed during any call or replying to any email. No form on any site consents to texts, because the numbers do not send them, and "reply STOP to any text" points at a mechanism that does not exist. Canonical templates: COMPLIANCE_STANDARDS.md. **The field and the consent move in the same commit, in both directions:** consent naming a channel the form does not collect is over-disclosure, and a form collecting a channel the consent does not name is contacting people on a channel they never agreed to. The second is the worse one.
14. **Sweep the copy that describes the fields.** The privacy page's collected-data list and its how-you-hear-back section, any how-it-works step, the form's own lead-in line, any "put it in your message" instruction, and the contact page's meta description all have to match the six fields. Live sites have shipped a privacy policy pointing at a free-text box that no longer existed, and the 2026-09-14 sweep found four separate places on one site still describing a four-field form, including a meta description reading "Four fields, no email". **Grep for the old field count and for "no email" before opening the pull request**, because the form component is the one place this change is obvious and every other place is silent.
...
| Multi-step or quote-builder forms | Still rejected, but the reason narrowed on 2026-09-14. The evidence says multi-step only outperforms at six or more fields, and the ask is now exactly six, so it sits on the line rather than clearly the wrong side of it. Build single-step until measured form volume on the six-field ask says otherwise; do not split a six-field form on theory |
