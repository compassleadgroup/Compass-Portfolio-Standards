...
3. **Check required flags and select defaults, not just the field count.** One live site had phone optional while email was required. Under item 1b every visible field is required, so the defect to look for now is any visible control without `required`, and any dropdown marked required whose default option carries a real value, which passes validation without the visitor ever choosing. The 2026-09-18 sweep made 222 such fixes across 59 sites, and 10 of those sites had a TCPA consent box that was not required at all.
